...
const int N = 4; // Number of particles in the world
const int d = 3; // d dimension world
double mass[N] = {1, 1, 1, 1}; // mass of each particles
const int Nu = 4; // Nucleus number
double nucl_pos[Nu][3] = {{4, 4, 0}, {4, -4, 0}, {-4, -4, 0}, {-4, 4, 0}}; // Nucleus position
...
double alpha = 10.0; // adaptive rate for birth-death process
...
void initialization(QMC_system &QMC){
	...
	for(int p = 0; p < N0_walker; p++){
		...
		for(int i = 0; i < N; i++){
			...
			for(int j = 0; j < d; j++)
				R[i].push_back(nucl_pos[i][j] + Gauss(generator));
		}
	}
}
